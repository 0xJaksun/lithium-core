...
import { NotFoundError, createClusterService, createEntryService } from "@lithium-ai/core";
import { DrizzleClusterAdapter } from "../cluster/cluster.port";
import { DrizzleEntryAdapter } from "./entry.port";
import { setupTestDb } from "../test/setup";

describe("Drizzle Entry Integration", async () => {
  const db = await setupTestDb();
  const clusterService = createClusterService(new DrizzleClusterAdapter(db));
  const entryService = createEntryService(new DrizzleEntryAdapter(db));
  ...
  it("should exclude entries from unrequested clusters", async () => {
    ...
    // Act
    const result = await entryService.list({ clusterIds: [requested.cluster.id] });
    if (!result.success) throw new Error("List failed");
  ...
  it("should return empty array for empty clusterIds", async () => {
    // Act
  ...
  it("should pair entries with their latest versions in listWithLatestVersion", async () => {
    ...
    // Act
    const result = await entryService.listWithLatestVersion({ clusterIds: [cluster.id] });
